...
const FilterIphone = document.getElementById('filter-iphone');

function render(list) {
  if (!FilterIphone) return;
  
  FilterIphone.innerHTML = "";

  const iphone = list.filter(p => p.title.toLowerCase().includes("iphone"));
  
  if (iphone.length === 0) {
    FilterIphone.innerHTML = "<p>No Products</p>";
    return;
  }
...
MegaMenuInput.addEventListener("input", () => {
  ...
  filtered.forEach(p => {
    ...
    <div class="MegaMenuContainer">
      <a  href="/src/product-details.html?id=${p.id}" class="product-link" style="text-decoration: none; color: inherit; display: flex; width: 100%;">
